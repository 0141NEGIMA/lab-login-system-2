    ...
    for member in all_members:
        ...
        if target_ping_status == 1: # 疎通成功したら
            if target_notion_status == "入室": # 元々入室していた人は累計分カウントを追加
                ...
                print(f"INFO[{datetime.now()}]: {member['name']} is still in the room.")
            elif target_notion_status == "退室": # 新たに入室した人は更新
                nt.enter_room(target_notionid)
                print(f"INFO[{datetime.now()}]: {member['name']} has entered the room.")
        else: # 疎通失敗したら
            if target_notion_status == "入室": # 新たに退室した人は更新
                nt.leave_room(target_notionid)
                print(f"INFO[{datetime.now()}]: {member['name']} has left the room.")
            elif target_notion_status == "退室":
                print(f"INFO[{datetime.now()}]: {member['name']} is still out of the room.")
